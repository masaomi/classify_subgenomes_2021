#!/usr/bin/env ruby
# encoding: utf-8
# Version = '20190122-145435'

require 'csv'

A_HOMEOLOG_RATIO = 0.5

unless genome_fa=ARGV[0] and homeolog_list_dat=ARGV[1]
  puts <<-eos
  usage:
    #{File.basename(__FILE__)} [genome.fa] [homeolog_list.dat]
  note:
...
CSV.foreach(homeolog_list_dat, :headers=>true, :col_sep=>"\t") do |row|
  ...
  scaff_a_gene = a_gene.gsub(/maker-/, '').gsub(/augustus_masked-/, '').gsub(/_obj.+/, '').gsub(/-augustus-gene-.+/, '').gsub(/-processed-gene-.+/, '').gsub(/-exonerate_.+/, '').gsub(/-est_.+/, '')
  scaff_b_gene = b_gene.gsub(/maker-/, '').gsub(/augustus_masked-/, '').gsub(/_obj.+/, '').gsub(/-augustus-gene-.+/, '').gsub(/-processed-gene-.+/, '').gsub(/-exonerate_.+/, '').gsub(/-est_.+/, '')
